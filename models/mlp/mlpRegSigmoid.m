function [model, L] = mlpRegSigmoid(X, y, k, lambda)
% Train a multilayer perceptron neural network for regression using sigmoid
% function as the last layer and tanh as activation function
% 
% Input:
%   X: d x n data matrix
...
% Adapted from original MLP model written by Mo Chen from PRML library.
if nargin < 4
    lambda = 1e-2;
end
eta = 1e-5;
...
%     forward
    ...
    Z{T+1} = sigmoid(W{T}'*Z{T}+b{T});                       % 5.114

%     loss
    E = Z{T+1}-y;
    Wn = cellfun(@(x) dot(x(:),x(:)),W);            % |W|^2
    L(iter) = dot(E(:),E(:))+lambda*sum(Wn);        % regularization;
