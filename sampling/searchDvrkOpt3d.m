function [v, maxInd, numChecks] = searchDvrk3d(X, Y, Z, V, s)
...
maxInd = floor(size(X) / 2); % initialization
numChecks = 0;
while s >= 1
...
%     [v, ind] = max(Vq(indices));
    ...
    center = floor(size(X) / 2); % Center of the window;
    offset = [i, j, k] - center; 
    
    maxInd = maxInd + offset; 
