% Script for finding optimal pose based on a single metric, e.g.
% reachability, self_collision, or env_collision. 
close all; clear
rng(0);
init;
...
%% Load Dataset
[X_train, y_train, X_test, y_test] = loadDvrk(input_path, dataset, n, true, true);
...
for i=1:size(X, 1)
    x0 = X_init(i, :);
    x = findPoseSingle(x0, xmin, xmax, mdl, scale_input, scale_output);
