...
%% Load Dataset
[X_reach, y_reach] = loadDvrk2(input_path, reachability_dataset, n, false);
[X_self_collision, y_self_collision] = loadDvrk2(input_path, self_collision_dataset, n, false);
[X_env_collision, y_env_collision] = loadDvrk2(input_path, environment_collision_dataset, n, false);
...
%% Train the models
self_collision_param = trainRVM(X_train, y_self_collision_train, X_test, y_self_collision_test, g);
env_collision_param = trainRVM(X_train, y_env_collision_train, X_test, y_env_collision_test, g);
reachability_param = trainRVM(X_train, y_reach_train, X_test, y_reach_test, g);
...
% labels
K_uniform = exp(-g*pdist2(X_uniform, X_train).^2);
y_uniform_reachability = rvmPredict(K_uniform, reachability_param);
y_uniform_collision = rvmPredict(K_uniform, self_collision_param);
y_uniform_env_collision = rvmPredict(K_uniform, env_collision_param);
...
%% Calculate losses
K_test = exp(-g*pdist2(X_test, X_train).^2);
y_self_collision_pred = rvmPredict(K_test, self_collision_param);
...
y_env_collision_pred = rvmPredict(K_test, env_collision_param);
...
y_reach_pred = rvmPredict(K_test, reachability_param);
...
K = exp(-g*pdist2(X, X_train).^2);
y_reach_grid = rvmPredict(K, reachability_param);
figure('Position', [327 87 800 800]);
...
% Self Collision
y_collision_grid = rvmPredict(K, self_collision_param);
subplot(2,2,2)
...
% Env Collision
y_env_collision_grid = rvmPredict(K, env_collision_param);
subplot(2,2,3)
...
cm = jet(256);
subplot(2,2,1)
colorScatter3(X_uniform(y_uniform_reachability>thres,1),X_uniform(y_uniform_reachability>thres,2),...
    X_uniform(y_uniform_reachability>thres,3),y_uniform_reachability(y_uniform_reachability>thres), cm);
view([153 58]); axis square; grid on;
xlabel('X');
ylabel('Y');
zlabel('\theta');
...
% Self Collision
subplot(2,2,2)
colorScatter3(X_uniform(y_uniform_collision>thres ,1),X_uniform(y_uniform_collision>thres,2),...
    X_uniform(y_uniform_collision>thres,3),y_uniform_collision(y_uniform_collision>thres), cm);
view([153 58]); axis square; grid on;
xlabel('X');
ylabel('Y');
zlabel('\theta');
...
% Env Collision
subplot(2,2,3)
colorScatter3(X_uniform(y_uniform_env_collision>thres ,1),X_uniform(y_uniform_env_collision>thres,2),...
    X_uniform(y_uniform_env_collision>thres,3),y_uniform_env_collision(y_uniform_env_collision>thres), cm);
view([153 58]); axis square; grid on;
xlabel('X');
ylabel('Y');
zlabel('\theta');
...
combined_score_uniform = y_uniform_reachability + y_uniform_collision + y_uniform_env_collision;
subplot(2,2,4)
colorScatter3(X_uniform(y_uniform_collision>thres & y_uniform_reachability > thres & y_uniform_env_collision > thres,1),...
              X_uniform(y_uniform_collision>thres & y_uniform_reachability > thres & y_uniform_env_collision > thres, 2),...
              X_uniform(y_uniform_collision>thres & y_uniform_reachability > thres & y_uniform_env_collision > thres,3), ...
              combined_score_uniform(y_uniform_collision>thres & y_uniform_reachability >thres & y_uniform_env_collision > thres),...
              cm);
view([153 58]); axis square; grid on;
xlabel('X');
ylabel('Y');
...
for i=1:size(X_out, 1)
    x0 = X_init(i, :);
    tic();
    [x,fval,exitflag, output] = find_pose_combined_rvm(x0, X_train, g, xmin, xmax, self_collision_param, reachability_param,...
        env_collision_param, scale_input, scale_output_collision, scale_output_reach);
    total_time = total_time + toc();
    
    % print scores; 
    K_x = exp(-g*pdist2(scale_input(x),X_train).^2);
    reachability_score = clip(rvmPredict(K_x, reachability_param),0.0001);
    collision_score = clip(rvmPredict(K_x, self_collision_param),0.0001);
    env_collision_score = clip(rvmPredict(K_x, env_collision_param),0.0001);
